#include <stdio.h> // подключаем работу с файлами
#include "fajl.h" // подключаем наш заголовок с прототипами
...
int read_parents(struct parent *parents, int max_parents) // читаем родителей из файла
{
    ...
    {
        return 0; // если открыть не получилось, возвращаем 0
    }
...
int write_records(struct record *records, int record_count) // записываем учеников обратно в файл
{
    ...
    fclose(file); // закрываем файл
    return 1; // возвращаем 1 чтобы показать успешную запись
} // конец функции write_records
