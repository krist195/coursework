#include <stddef.h> // подключаем определение NULL
#include "stroki.h" // подключаем прототипы функций
...
int text_compare(const char *a, const char *b) // сравниваем строки символ за символом
{
    ...
    if (a == NULL && b == NULL) // обрабатываем случай двух пустых указателей
    {
        return 0; // строки считаем равными
    }
    if (a == NULL) // если первая строка отсутствует
    {
        return -1; // считаем что пустая строка меньше
    }
    if (b == NULL) // если вторая строка отсутствует
    {
